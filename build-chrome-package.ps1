# Chrome Extension Packaging Script (PowerShell)
...
# Configuration
$OutputDir = "releases"
$OutputFile = "confluence-chrome.zip"
...
$ExcludePatterns = @(
    ...
    # Chrome manifest source
    "manifest.chrome.json"
)

Write-Host "🌐 Creating Chrome Extension Package..." -ForegroundColor Cyan
Write-Host ""

# Check if manifest.chrome.json exists
if (-not (Test-Path $ManifestSource)) {
    Write-Host "❌ Error: $ManifestSource not found." -ForegroundColor Red
    Write-Host "Please ensure manifest.chrome.json exists in the root directory." -ForegroundColor Red
    exit 1
}
...
$TempDir = Join-Path $env:TEMP "chrome-extension-build-$(Get-Date -Format 'yyyyMMddHHmmss')"
...
try {
    ...
    Write-Host ""
    Write-Host "✅ Chrome extension package created successfully!" -ForegroundColor Green
    Write-Host "📦 File: $OutputPath" -ForegroundColor Cyan
    Write-Host "📊 Size: $([math]::Round($zipSize, 2)) MB" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "🚀 Ready to upload to Chrome Web Store: https://chrome.google.com/webstore/devconsole" -ForegroundColor Yellow
} catch {
